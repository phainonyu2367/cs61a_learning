...
def roll_dice(num_rolls, dice=six_sided):
    ...
    current = 0
    sum = 0
    for count in range(0, num_rolls):
        current = dice()
        if current == 1:
            Pig_Out_flag = True
        sum += current
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # END PROBLEM 3
# it is fantastic to see what the profs wants to exactly show us
# devide your goals into simple functions and oraganize them together
# in a easy way but accomplish complex abilities
...
def swine_align(player_score, opponent_score):
    ...
    if gcd < 10: return False
    while ((player_score % gcd != 0) | (opponent_score % gcd != 0)): 
        gcd -= 1
    if gcd < 10: return False
    return True
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def comp(score0, score1, who = who, last_score = last_score, running_high = running_high):
        if who == 0: temp = score0
        else: temp = score1
        if temp - last_score > running_high: 
            running_high = temp - last_score
            print(f"{running_high} point(s)! The most yet for Player {who}")
        return announce_highest(who, temp, running_high)
    return comp
        # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    # END PROBLEM 8
#this only exposes that you havn't understood iteration yet.
...
def final_strategy(score, opponent_score):
    ...
    considering that free_bacon's result is forseeable while 
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return extra_turn_strategy(score, opponent_score, 11, 5) 
    # END PROBLEM 12
